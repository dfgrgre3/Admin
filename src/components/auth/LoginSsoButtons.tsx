"use client";

import { Button } from "@/components/ui/button";
import { Separator } from "@/components/ui/separator";

interface LoginSsoButtonsProps {
  onSelect: (provider: "google" | "microsoft" | "saml") => void;
}

/** SSO provider buttons shown on the credentials step of `LoginPage`. */
export default function LoginSsoButtons({ onSelect }: LoginSsoButtonsProps) {
  return (
    <>
      <div className="relative my-6">
        <div className="absolute inset-0 flex items-center">
          <Separator className="bg-white/10" />
        </div>
        <div className="relative flex justify-center text-sm">
          <span className="bg-gray-900 px-4 text-gray-500">أو</span>
        </div>
      </div>

      <div className="grid grid-cols-3 gap-3">
        <Button
          type="button"
          variant="outline"
          onClick={() => onSelect("google")}
          className="h-11 bg-white/5 border-white/10 text-white hover:bg-white/10"
        >
          <svg className="ml-2 h-5 w-5" viewBox="0 0 24 24">
            <path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
            <path fill="currentColor" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
            <path fill="currentColor" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
            <path fill="currentColor" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
          </svg>
          Google
        </Button>
        <Button
          type="button"
          variant="outline"
          onClick={() => onSelect("microsoft")}
          className="h-11 bg-white/5 border-white/10 text-white hover:bg-white/10"
        >
          <svg className="ml-2 h-5 w-5" viewBox="0 0 24 24">
            <path fill="#f3f3f3" d="M0 0h24v24H0z" />
            <path fill="#f35325" d="M1 1h10v10H1z" />
            <path fill="#81bc06" d="M13 1h10v10H13z" />
            <path fill="#05a6f0" d="M1 13h10v10H1z" />
            <path fill="#ffba08" d="M13 13h10v10H13z" />
          </svg>
          Microsoft
        </Button>
        <Button
          type="button"
          variant="outline"
          onClick={() => onSelect("saml")}
          className="h-11 bg-white/5 border-white/10 text-white hover:bg-white/10"
        >
          <svg className="ml-2 h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
            <path d="M12 2L2 7v10l10 5 10-5V7L12 2zm0 2.18L20 8.5v7l-8 4-8-4v-7l8-5.82z" />
            <path d="M12 6L5.5 9.5v5L12 18l6.5-3.5v-5L12 6z" />
          </svg>
          SAML
        </Button>
      </div>
    </>
  );
}
